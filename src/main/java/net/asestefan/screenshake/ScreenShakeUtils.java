...
@Author = ASEStefan
 */
public class ScreenShakeUtils extends Entity {
    private static final EntityDataAccessor<Float> RADIUS = SynchedEntityData.defineId(ScreenShakeUtils.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Float> MAGNITUDE = SynchedEntityData.defineId(ScreenShakeUtils.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Integer> DURATION = SynchedEntityData.defineId(ScreenShakeUtils.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> FADE_DURATION = SynchedEntityData.defineId(ScreenShakeUtils.class, EntityDataSerializers.INT);

    public ScreenShakeUtils(EntityType<?> type, Level world) {
        super(type, world);
    }

    public ScreenShakeUtils(Level world, Vec3 position, float radius, float magnitude, int duration, int fadeDuration) {
        super(ScreenShakeRegistry.SCREEN_SHAKE.get(), world);
        setRadius(radius);
    ...
    public float getShakeAmount(Player player, float delta) {
        ...
        float distFrac = (float) (1.0f - MathUtils.clampedDouble(position().distanceTo(playerPos) / getRadius(), 0, 1));
    ...
    public static void ScreenShake(Level world, Vec3 position, float radius, float magnitude, int duration, int fadeDuration) {
        if (!world.isClientSide) {
            ScreenShakeUtils ScreenShake = new ScreenShakeUtils(world, position, radius, magnitude, duration, fadeDuration);
